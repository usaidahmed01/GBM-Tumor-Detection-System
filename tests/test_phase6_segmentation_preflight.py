...
def make_study(session: Session, patient_id: str):
    ...
    return create_study(session, StudyCreate(assessment_uuid=assessment.id))


def make_dicom_ready(session: Session, patient_id: str):
    study = make_study(session, patient_id)
    study.source_format = SourceFormat.DICOM
    study.modality = "MR"
    study.deidentification_status = DeidentificationStatus.METADATA_DEIDENTIFIED
    study.deidentified_storage_key = f"studies/{study.id}/derived/dicom/test.zip"
    ...
    study.qc_summary = {
        ...
        "partial_reasons": [],
        "fail_reasons": [],
        "warnings": ["DICOM_PIXEL_PRIVACY_NOT_FORMALLY_VALIDATED"],
...
def make_nifti_ready(session: Session, patient_id: str):
    study = make_study(session, patient_id)
...
def test_segmentation_contract_freezes_model_channel_order_and_scope():
    assert SEGMENTATION_INPUT_CHANNEL_ORDER == ("T1C", "T1", "T2", "FLAIR")
    assert SEGMENTATION_OUTPUT_CHANNEL_ORDER == ("TC", "WT", "ET")
    assert SEGMENTATION_MODEL_CONTRACT.reference_spacing_mm == (1.0, 1.0, 1.0)
    assert SEGMENTATION_MODEL_CONTRACT.runtime_model_loading_implemented is True
    assert SEGMENTATION_MODEL_CONTRACT.inference_implemented is True
    assert SEGMENTATION_MODEL_CONTRACT.clinical_validation_claimed is False


def test_eligible_dicom_builds_model_order_without_starting_inference(session):
    study = make_dicom_ready(session, "GBM-P6-DICOM")
    result = build_segmentation_preflight(session, study)
    assert result["status"] == "ready_for_preprocessing"
    assert [item["sequence"] for item in result["channels"]] == ["T1C", "T1", "T2", "FLAIR"]
    assert [item["channel_index"] for item in result["channels"]] == [0, 1, 2, 3]
    assert all(item["source_kind"] == "dicom_series" for item in result["channels"])
    assert result["model_execution_started"] is False
    assert result["segmentation_generated"] is False


def test_eligible_nifti_uses_confirmed_opaque_volume_indices_in_model_order(session):
    study = make_nifti_ready(session, "GBM-P6-NIFTI")
    result = build_segmentation_preflight(session, study)
    assert [item["sequence"] for item in result["channels"]] == ["T1C", "T1", "T2", "FLAIR"]
    assert [item["volume_index"] for item in result["channels"]] == [1, 0, 2, 3]
    assert all(item["source_kind"] == "nifti_volume" for item in result["channels"])
    assert result["physical_volume_generated"] is False
    assert result["anatomical_localization_generated"] is False


def test_preflight_rejects_stale_phase5_routing(session):
    study = make_nifti_ready(session, "GBM-P6-STALE")
    study.capability_routing_status = CapabilityRoutingStatus.PENDING
    study.capability_summary = {"stale": True}
    study.status = StudyStatus.UPLOADED
    session.commit()
    with pytest.raises(SegmentationPreflightError, match="not ready for analysis"):
        build_segmentation_preflight(session, study)


def test_preflight_rejects_standalone_image_even_if_2d_branch_is_ready(session):
    study = make_study(session, "GBM-P6-IMAGE")
    ...
    session.commit()
    route_study_capabilities(session, study)
    with pytest.raises(SegmentationPreflightError, match="only DICOM or NIfTI"):
        build_segmentation_preflight(session, study)


def test_preflight_never_creates_analysis_run(session):
    study = make_dicom_ready(session, "GBM-P6-NORUN")
